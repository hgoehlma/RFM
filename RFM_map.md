...
`v0.11.3` // `top_level_artifact` // [living]

---

## What this document covers

Where every document in RFM sits, what version it currently carries, and whether it travels in a release. This is the navigational entry point for the document set. A reader arriving at RFM starts here to find which document to open. `README.md` is the entry point for a reader deciding whether RFM is worth reading at all, and carries no versions.

This document does not list itself. A file cannot record its own version twice without one copy going stale.
...
**Travels** records whether a document is part of the set handed to a practitioner in a release, not whether it is available. Everything in this repository is public. `stays` means the document is not part of the handed set, because a session reading only that set must not need it. This map travels too, as the template a practitioner's LLM works from; it has no row because the map does not list itself.

A travel value is a ruling, not a judgment a session makes while adding a row. A new document's cell is decided with the human.

| Document | Type | Version | Travels | What it carries |
|---|---|---|---|---|
| `RFM_top_level_reasoning.md` | `top_level_reasoning` | v0.24.0 | travels | The problem, assumptions, landscape, chosen direction, boundaries, open questions and hard lessons governing the methodology as a whole |
...
| `RFM_glossary.md` | `top_level_artifact` | v0.4.0 | travels | Disambiguation of terms that carry different meanings across reader contexts |
...
| `RFM_instruction_design_reasoning.md` | `module_reasoning` | v0.18.0 | stays | The reasoning document governing how an instruction is shaped, where it is placed, and whether anything checks it |
| `RFM_prompts_reasoning.md` | `module_reasoning` | v0.5.1 | stays | The reasoning document governing all system prompt decisions |
| `RFM_traveling_prompt_reasoning.md` | `module_reasoning` | v0.2.2 | stays | The reasoning document governing traveling prompt design decisions |
...
| `RFM_sweep_reasoning.md` | `module_reasoning` | v0.7.0 | stays | The reasoning document governing sweep prompt design decisions |
...
| `RFM_human_prompt_reasoning.md` | `module_reasoning` | v0.2.1 | stays | Reasoning document governing human prompt design decisions |
...
| `RFM_guided_drafting_prompt_reasoning.md` | `module_reasoning` | v0.1.1 | stays | Reasoning document governing guided drafting prompt design decisions |
...
| `rfm-unslop.skill` | `skill_artifact` | unversioned | travels | The packaged skill: strips LLM tells from session output before it leaves the model, applied to every response |
...
| `examples/` |  | unversioned | stays | Reasoning documents from projects using RFM, kept as worked examples |
