...
`v0.14.0` // `module_reasoning` // [living]
...
**[AS-DECAY] - Adherence to an instruction falls as a conversation continues, even though a stateless architecture keeps the instruction present in every later turn's context.** The effect has been measured across three model generations. A single standing constraint loses 26% of its adherence over fifty turns, and where constraints accumulate the loss runs from 38% to 63%. Retention of a first-turn instruction was 58.57% for the strongest model of an earlier frontier set and below 35% for most of the rest. The current frontier generation was tested in 2026 and shows the same shape: constraint adherence falls from 3.63 in single-shot use to 3.31 under multi-turn pressure, failures cluster at particular conversation depths rather than accumulating smoothly, and recovery after a failure stays below 30% for every model tested. The assumption breaks if a model holds a standing constraint flat across a long conversation without any re-presentation, because presence alone would then be enough, and whether an instruction still holds would not be a separate variable to design for. Why presence stops binding is not settled. One 2026 result finds models restating a constraint correctly 97.3% of the time while violating it, with five of seven models violating a constraint they could state at better than even odds, which places the failure in whether the model treats the constraint as governing rather than in whether the model can retrieve it. A separate paper on constraint handoff across agent workflow stages found that a constraint carried forward as information rather than restated as a requirement loses its force, with standard compression producing complete deactivation of the safety blockers tested (Sun et al., arXiv 2608.24569, August 2026). Read together, these point to a single candidate mechanism: what decays is not presence but hold, an instruction present but no longer read as a live requirement, and re-presentation restores adherence only when it restates the instruction as binding rather than carrying it forward as summary. Neither result was designed to test this directly. Sun et al.'s finding is about handoffs between workflow stages, not decay within one continuous session, and the restatement-accuracy study does not test re-presentation at all. Until something tests that directly, this is the leading candidate, not a confirmed mechanism.
...
|---|---|---|
| Always-loaded context file | A file is injected into context at the start of every session, whole. `AGENTS.md` and a project-root `CLAUDE.md` work this way, as does a platform's project instruction field. | Stays present at every later moment under a stateless architecture, but staying present does not mean it still binds. Adherence to an instruction presented early falls as the conversation continues, measured at a 26% drop from first to last turn for a single constraint across fifty turns, and 38% to 63% where constraints accumulate. Measured separately as retention of an instruction given in the first turn, the strongest frontier model tested scored 58.57% and most scored below 35%. The effect was measured again in 2026 on the then-current frontier models. The instruction never leaves context and stops binding anyway, and nothing errors. |
...
**[OQ-INTV] Whether interval is the right unit, given that presence persists but hold decays.**

Re-presentation is the only decay mitigation a practitioner can apply. Nothing establishes how often it must happen. Every carrier RFM uses presents once at the start of work or presents on a trigger, and neither of those is an interval. The cost of choosing a short one is context spent on every repetition. The cost of choosing a long one is an instruction that sits in context unread as binding, unnoticed for exactly the reason `[AS-DECAY]` gives: nothing errors when presence stops binding. `[AS-DECAY]`'s leading candidate mechanism, that re-presentation restores adherence only when it restates the instruction as a requirement rather than carrying it forward as summary (Sun et al., arXiv 2608.24569, August 2026), was measured across handoffs between agent workflow stages, not within one continuous session. Whether it holds there too is untested. If it does, the question stops being how often to re-present and becomes whether a given re-presentation is written to bind or written as a reminder, which would make this a question about a carrier's form rather than its frequency, the same axis `[AS-FORM]` already designs around for a different purpose. If it does not hold within a session, interval remains the live variable and this stands as before.

**[OQ-SBART] Whether RFM specifies an artifact type for session-boundary delivery, or only requires that the boundary be answered.**
...
**[HL-PRIMCHK] A reasoning document can rest on an unverified mechanical claim for multiple sessions, because nothing forces a check against a primary source once the claim reads as plausible.**

This document's carrier taxonomy treated a carrier's load point as governing whether an instruction reaches a later moment, which assumes content not resent leaves context. Nobody had checked that assumption against how the underlying API actually behaves. It surfaced only when a glossary draft compressed the reasoning to one parenthetical, "the traveling prompt presents once at the start of work", and that compression made the claim checkable in a way the fuller prose had not. Checked directly against Anthropic's documentation: the Messages API is stateless and resends the full conversation history on every turn, so presence was never the variable in question; what an instruction loses with distance is its hold, not its place in context. The prose had read as settled reasoning for several sessions without anyone opening a primary source to confirm the one claim it depended on.

**[HL-EPIDRIFT] A document's closing epigraph is not re-derived when its body changes, so an edit to the body can leave the epigraph contradicting it.**

Correcting this document's Problem section and `[AS-DECAY]` left the closing epigraph, two lines written to summarize the old framing, restating the exact claims just corrected, "an instruction reaches only the moments its carrier reaches" and "silence is invisible from inside the instruction", verbatim. Nothing flagged the mismatch; it was caught only because the full file was reread after the edit rather than trusting the diff. An epigraph is drafted once, at a different time than any later body edit, and nothing reconciles the two automatically.

---

*module_reasoning // [living]*
*an instruction can lose its hold while still sitting in context*
*losing that hold is invisible from inside the instruction*
