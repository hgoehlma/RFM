...
RFM places the reasoning document at the center. Not the code. Not the output. The reasoning (explicit, hierarchical, curated) is designed to survive beyond the people and sessions that produced it, so that returning to it six months later, or bringing in a new collaborator, doesn't mean starting from scratch.
