...
**The First Session Guidance** is a short document the newcomer reads immediately before starting. Its job is narrow: surface the one failure mode most likely to undermine the session, and give the newcomer a concrete move to counter it. It is not an introduction to RFM. It is not a summary of the human prompt. It is session-specific preparation. It also confirms the newcomer has read the README and the top-level reasoning document before the session begins; the guided drafting prompt assumes that orientation and does not supply it.
